...
void TabPrinter::update_machine_limits_description(const MachineLimitsUsage usage)
{
    ...
    //update fields used
    if (usage > MachineLimitsUsage::TimeEstimateOnly) {
        wxColour black(0, 0, 0);
        Field* field = m_active_page->get_field("machine_max_feedrate_x");
        if (field) dynamic_cast<wxTextCtrl*>(field->getWindow())->Disable();
    }
    /*
    append_option_line_kinematics(optgroup, "machine_max_feedrate_" + axis);
    else
